...
__export(src_exports, {
  VMdPreview: () => preview_default,
  VueMarkdownKit: () => VueMarkdownKit,
  createGithubTheme: () => createGithubTheme,
  createMarkdownRenderer: () => createMarkdownRenderer,
  default: () => src_default,
  githubTheme: () => github_default,
...
var VMdPreview = {
  ...
  methods: {
    ...
    handlePreviewClick(event) {
      ...
      const images = Array.from(this.$el.querySelectorAll("img")).map((image) => image.getAttribute("src")).filter(Boolean);
...
var githubTheme = {
  install(VMdPreview2, options = {}) {
    VMdPreview2.theme(createGithubTheme(options));
  }
};
...
// src/index.js
var VueMarkdownKit = {
  version: preview_default.version,
  install(Vue) {
    preview_default.install(Vue);
  }
};
var src_default = VueMarkdownKit;
